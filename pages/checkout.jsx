...
function Checkout(props) {
  ...
  useEffect(() => {   
    console.log("mira el isAuth", props.isAuth);
    if (!props.isAuth) {
      router.push("/login");
    }
  }, []);
  ...
  const finalizarCompra = (e) => {
    ...
    .then((res) => {
      ...
      swal({
        text: "No tienes productos en el carrito. Asegurate de agreagar al menos un producto",
        icon: "warning",
      });
    }
  ...
  if(!props.isAuth) return <h1>Cargando...</h1>  
    ...
    <>
      {!orderId ? (    
        ...
        <div className={style.cajaTextoCompra}>
          <p className={style.parrafocompra1}>
            Muchas gracias por tu compra. Te enviaremos un correo cuando tu
            pedido salga de nuestro almacén.
          </p>
          <p className={style.parrafocompra2}>
            Tu código de referencia es:{" "}
            <strong className={style.strongId}>{orderId}</strong>
          </p>
          <Link href={`/`}>
            <a>
              <button className={style.btnAceptar}>Aceptar</button>
            </a>
          </Link>
        </div>
      )}   
...
export async function getServerSideProps(context) {
  console.log("mira el contexto");
  console.log(context.req.headers.cookie);
  const isAuth = context.req.headers.cookie?.split("=")[1]; // user=null || undefined
  const operacion = isAuth === "null" || typeof isAuth === "undefined";

  return {
    props: {
      isAuth: operacion ? false : true,
